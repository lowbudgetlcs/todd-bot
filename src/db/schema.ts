...
export const series = pgTable(
  "series",
  {
    ...
    team1_id: integer("team1_id").references(() => teams.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
    team2_id: integer("team2_id").references(() => teams.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
  ...
  (table) => {
    return {
      series_team1_id_team2_id_playoffs_key: unique(
        "series_team1_id_team2_id_playoffs_key",
      ).on(table.team1_id, table.team2_id, table.playoffs),
    };
  },
);
...
export const teams = pgTable(
  "teams",
  {
    ...
    captain_id: integer("captain_id").references(
  ...
  (table) => {
    return {
      lower_idx: index("teams_lower_idx").using(
        "btree",
        sql`lower((name)::text)`,
      ),
    };
...
export const divisions = pgTable(
  ...
  (table) => {
    return {
      lower_idx: index("divisions_lower_idx").using(
        "btree",
        sql`lower((name)::text)`,
      ),
      divisions_tournament_id_key: unique("divisions_tournament_id_key").on(
        table.tournament_id,
      ),
    };
  },
...
export const players = pgTable(
  ...
  (table) => {
    return {
      primary_riot_puuid_idx: uniqueIndex(
        "players_primary_riot_puuid_idx",
      ).using("btree", table.primary_riot_puuid),
      players_primary_riot_puuid_key: unique(
        "players_primary_riot_puuid_key",
      ).on(table.primary_riot_puuid),
      players_summoner_name_key: unique("players_summoner_name_key").on(
        table.summoner_name,
      ),
    };
  },
...
export const group_keys = pgTable(
  "group_keys",
  {
  ...
  (table) => {
    return {
      lower_idx: index("group_keys_lower_idx").using(
        "btree",
        sql`lower((letter)::text)`,
      ),
    };
  },
);
