...
describe('reading a code back off a message', () => {
  ...
  it('takes the code out of the message Todd printed it on', () => {
    // A real one, in full: region prefix, dash, UUID. 44 characters, which is
    // why it is read back off the message rather than carried in a custom_id.
    const real = 'NA04eff-c5b0b774-c049-47cf-88f4-eee05c8d83ae';
    expect(shortcodeIn(codeMessage(1, real))).toBe(real);
  });

  it('survives the team names and pings around it', () => {
    const real = 'NA050c5-695bbdae-f77a-4129-aae5-9f44c6d570bc';
    const printed =
      `# Game 1 \n 🟦 __**ZZZain's Test Team**__ v.s.  __**ZZZDEV TEAMs**__ 🟥\n` +
      `\nCode: \`\`\`${real}\`\`\`\n` +
      'Game Generated By: <@123>\nEnemy Captain: <@456>\n';
    expect(shortcodeIn(printed)).toBe(real);
  });
